...
TODO only metric space for limit points? or keep topological space (technically, metric is stronger than topology and topology was never used in the conditions)
...
\n \dfn{closure}\ml(\bar{S}, \ml(S, M, \mathcal{O}, d\mr)\mr) \iff \bar{S} = S \cup \{\rfr{limitPoint}\ml(p, \ml(S, M, \mathcal{O}, d\mr)\mr) \mid p \in M\}
...
\n \thm{} \rfr{innerProduct} \implies \rfr{norm} \implies \rfr{metric}
\n \thm{} \ml(\rfr{metric} \land \forall_{u, v, w \in V} \forall_{s \in \mathbb{R}} \ml(d\ml(s \ml(u + w\mr), s \ml(v + w\mr)\mr) = |s| d\ml(u, v\mr)\mr) \land \forall_{v \in V} \ml(|| v || = d\ml(v, \field{0}\mr)\mr)\mr) \implies 
\m \ml(\rfr{norm} \land \forall_{u, v \in V} \ml(2 || u ||^2 + 2 || v ||^2 = || u + v ||^2 + || u - v ||^2\mr) \land \forall_{v, w \in V} \ml(\langle v, w \rangle = \frac{|| v + w ||^2 - || v - w ||^2}{4}\mr)\mr) \implies 
\m \ml(\rfr{innerProduct}\mr)
\n \dfn{basis}\ml(\ml(b\mr)_n, \ml(V, +, \cdot, \mr)\mr) \iff \ml(\rfr{vectorSpace}\ml(\ml(V, +, \cdot\mr), \ml(\mr)\mr)\mr) \land
\m \ml(\forall_{v \in V} \exists_{\ml(a\mr)_n \in \mathbb{R}^n} \ml(v = \sum_{i = 1}^n \ml(a_i b_i\mr)\mr)\mr)
...
\n \dfn{denseMap}(L, (D, H, +, \cdot, \langle \$1, \$2 \rangle)) \iff (D \subseteq H) \land (\rfr{linearOperator}(L, (D, +, \cdot, H, +, \cdot)) \land
\m (\rfr{innerProductTopology}(\mathcal{O}, (H, +, \cdot, \langle \$1, \$2 \rangle))) \land (\rfr{dense}(D, (H, \mathcal{O}, d(\$1, \$2))))
\n \dfn{traceClass}(L, (H, +, \cdot, \langle \$1, \$2 \rangle)) \iff (\rfr{hilbertSpace}(((H, +, \cdot, \langle \$1, \$2 \rangle), ()), ())) \land 
\m (\rfr{linearOperator}(Tr(L), (L, H, +, \cdot, H, +, \cdot))) \land 
\m (\forall_{(b)_n \subseteq H} (\rfr{orthonormalBasis}((b)_n, (H, +, \cdot, \langle \$1, \$2 \rangle)) \implies Tr(L) = \sum_{n} (\langle b_n, L(b_n) \rangle) < \infty))
\m \eqComment{does not depend on the choice of orthonormal basis}
\m \eqComment{DEFINE}
\n \dfn{mapNorm}(|| L ||, (L, V, +_V, \cdot_V, || \$1 ||_V, W, +_W, \cdot_W, || \$1 ||_W)) \iff (\rfr{linearOperator}(L, (V, +_V, \cdot_V, W, +_W, \cdot_W))) \land
\m (\rfr{normedVectorSpace}((V, +_V, \cdot_V, || \$1 ||_V), ())) \land (\rfr{normedVectorSpace}((W, +_W, \cdot_W, || \$1 ||_W), ())) \land 
\m (|| L || = \rfr{sup}(\{\frac{|| L f ||_W}{|| f ||_V} \mid f \in V\}) = \rfr{sup}(\{|| L f ||_W \mid f \in V \land || f || = 1\}))
\n \thm{} || L f || \leq || L || || f ||
\n \thm{CONTINUE HERE 46:50}
\n \dfn{boundedMap}(|| L ||, (L, V, +_V, \cdot_V, || \$1 ||_V, W, +_W, \cdot_W, || \$1 ||_W)) \iff
\m (\rfr{mapNorm}(|| L ||, (L, V, +_V, \cdot_V, || \$1 ||_V, W, +_W, \cdot_W, || \$1 ||_W))) \land (|| L || < \infty)
\n \lnot \rfr{boundedMap}(|| L ||, (L, V, +_V, \cdot_V, || \$1 ||_V, W, +_W, \cdot_W, || \$1 ||_W)) \impliedby
\m (U \subset V) \land (\infty = \rfr{mapNorm}(|| L ||_U, (L, U, +_U, \cdot_U, || \$1 ||_U, W, +_W, \cdot_W, || \$1 ||_W)) \leq || L ||)
\n 0
\n \dfn{adjoint}(L^T, (L, V, +_V, \cdot_V, \langle \$1, \$2 \rangle_V, W, +_W, \cdot_W, \langle \$1, \$2 \rangle_W)) \iff (\rfr{hilbertSpace}((V, +_V, \cdot_V, \langle \$1, \$2 \rangle_V), ())) \land 
\m (\rfr{hilbertSpace}((W, +_W, \cdot_W, \langle \$1, \$2 \rangle_W), ())) \land (\rfr{linearOperator}(L, (V, +_V, \cdot_V, W, +_W, \cdot_W))) \land
\m (\forall_{v \in V} \forall_{w \in W} ((\langle L v, w \rangle_W = \langle v, L^T w \rangle_V) \lor ((L v)^T w = v^T L^T w)))
\m \eqComment{target operator that is equivalent to the domain operator}
\n \dfn{selfAdjoint}(L, (V, +_V, \cdot_V, \langle \$1, \$2 \rangle_V, W, +_W, \cdot_W, \langle \$1, \$2 \rangle_W))) \iff
\m L = \rfr{adjoint}(L^T, (L, V, +_V, \cdot_V, \langle \$1, \$2 \rangle_V, W, +_W, \cdot_W, \langle \$1, \$2 \rangle_W))
...
\n \thm{Spectral thm.} (\rfr{matrix}(L, (n, n))) \land (\rfr{selfAdjoint}(L, (\mathbb{R}^n, +, \cdot, \langle \$1, \$2 \rangle, \mathbb{R}^n, +, \cdot, \langle \$1, \$2 \rangle))) \implies
\m (E = \{v \in \mathbb{R}^n \mid \rfr{eigenvector}(v, (L, \mathbb{R}^n, +, \cdot))\}) \land (\rfr{orthonormalBasis}(E, (\mathbb{R}^n, +, \cdot, \langle \$1, \$2 \rangle)))
\m \eqComment{wow}
\end{align}
\end{tcolorbox}
