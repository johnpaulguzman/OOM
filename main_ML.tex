...
\n \dfn{eigenvalue}\ml(\lambda, \ml(v, L, V, +, \cdot\mr)\mr) \iff \forall_{s \in \mathbb{R} \setminus \{0\}} \ml(\rfr{eigenvector}\ml(s v, \ml(L, V, +, \cdot\mr)\mr)\mr)
...
\n \dfn{decomposeLU}\ml(LU\ml(A\mr), \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, n\mr)\mr)\mr) \land \ml(\exists_E \ml(E A = U\mr)\mr) \land \ml(\forall_{x < n} \forall_{0 < i < x} \ml(U_{i, i} = 0\mr)\mr) \land 
...
\m \ml(0 = \frac{x^T x + y^T y - \ml(x^T x + y^T y\mr)}{2} = \frac{x^T y + y^T x}{2} = x^T y\mr) 
...
\n \thm{} P = P^T = P^2
\n \thm{} \rfr{independent}(A) \implies invertible(A^T A)
...
\m \eqComment{DEFINE}
%\m \eqComment{signed perimeter? parallelepiped spanned by the operator}
\n \thm{} A = A^T = A^2 \implies Tr(A) = \rfr{dimensionality}(N, (A)) \eqComment{counts dimensions} % http://www.onmyphd.com/?p=eigen.decomposition
\n \dfn{invertibleOperator}(A, (V, +, \cdot, || \$1 ||)) \iff (det(A) \neq 0) \lor (\rfr{independentOperator}(A, ()))
%\n \textbf{from def: $Ker(A) \perp Img(A^T)$}
%\n \textbf{normal equation: nearest solvable both from $A^T A$ and partial derivatives??}
\n TODO FIX INCOSISTENT PARAMETERS, formal eigenbasis diagonal matrix
\n CONSOLIDATE INVERTIBLE IDNEPENDENT DFNS CONTHERE
\n \dfn{eigenDecomposition}(S \Lambda S^{-1}, (A, V, +, \cdot, || \$1 ||)) \iff (S \in (\rfr{eigenvectors}(X, (A, V, +, \cdot, || \$1 ||))^n)^T) \land 
\m (\exists_\Lambda \forall_{p \in \mathbb{N}}(\Lambda^p \{1\}^n = \{\lambda^p \mid \rfr{eigenvalue}(XXX)\})) \land
\m (\rfr{invertibleOperator}(S, ()) \implies \exists_{S^-1} (A S = S \Lambda \implies A = S \Lambda S^{-1}))
\n A x = \lambda x
