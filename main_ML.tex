...
\n \dfn{diagonalOperator}(A, ()) \iff (\rfr{symmetricOperator}(A, ())) \land (\rfr{triangularOperator}(A, ())) 
...
\m (\rfr{independentOperator}(S, ()) \implies \exists_{S^-1} (A S = S \Lambda \implies A = S \Lambda S^{-1}))
\n \thm{} \rfr{eigenDecomposition}(S \Lambda S^{-1}, (A, V, +, \cdot, || \$1 ||)) \implies A^2 = (A) (A) = S \Lambda S^{-1} S \Lambda S^{-1} = S \Lambda^2 S^{-1}
\n \thm{} \dfn{spectralDecomposition}(Q \Lambda Q^T, (A, V, +, \cdot, || \$1 ||)) \iff (\rfr{symmetricOperator}(A, ()) \land A = \overline{A}^T) \implies
...
\n \text{SVD, AV eq USigma , A orthonormspace eq orthonormcolspace diaglambda}
\m \text{orthonormal basis that A maps to orthonormal basis scaled by eigenvalues}
\n \dfn{singularValueDecomposition}(Q \Sigma R^T, (A)) \iff (\rfr{orthogonalOperator}(Q, (V, +, \cdot, \$1^T \$2))) \land 
\m (\rfr{orthogonalOperator}(R, (V, +, \cdot, \$1^T \$2))) \land (\rfr{semiPositiveDefiniteOperator}(\Sigma, (V, +, \cdot, \$1^T \$1))) \land
\m (A V = U \Sigma) \land (A = U \Sigma V^{-1} = U \Sigma V^T) \land (A^T A = V \Sigma^T U^T U \Sigma V^T = V \Sigma^T \Sigma V^T)
\m let V be normed eigendecomp CONTHERE
