...
\n \dfn{generatedSigmaAlgebra}\ml(\sigma\ml(\mathcal{M}\mr), \ml(\mathcal{M}, S\mr)\mr) \iff \ml(\forall_{M \in \mathcal{M}} \ml(\rfr{sigmaAlgebra}\ml(M, \ml(S\mr)\mr)\mr)\mr) \land 
\m \ml(\rfr{sigmaAlgebra}\ml(\sigma\ml(\mathcal{M}\mr), \ml(S\mr)\mr) = \bigcap \ml(\mathcal{M}\mr)\mr)
...
\n \dfn{randomVariable}(X, (\Omega, \mathcal{F}, P)) \iff \ml(\rfr{probabilitySpace}\ml(\Omega, \mathcal{F}, P\mr)\mr) \land (\rfr{map}(X, (\Omega, \mathbb{R}))) \land
\m (\rfr{measurableMap}(X, (\Omega, \mathcal{F}, \mathbb{R}, \rfr{euclideanSigma}(\sigma_S, ()))))
\m \eqComment{Random-Deterministic Variable-Function maps the measurable space to the real line and borel sets}
\n \dfn{PL}(P_X, (X, \Omega, \mathcal{F}, P)) \iff (\rfr{randomVariable}(X, (\Omega, \mathcal{F}, P))) \land 
\m (\forall_{B \in \sigma_S} (P_X(B) = P(\rfr{preimage}(A, (B, X, \Omega, \mathbb{R})) = (P \circ X^{-1}) (B) ) = P(X \in B)))
\m \eqComment{probability of outcomes occuring in the Borel set}
\n \dfn{piSystem}(\mathcal{G}, (\Omega)) \iff \mathcal{G} \subseteq \mathcal{P}(\Omega) \land \forall_{A, B \in \mathcal{G}} (A \cap B \in \mathcal{G})
\n \thm{} (\rfr{piSystem}(\mathcal{G}, (\Omega)) \land \mathcal{F} = \sigma(\mathcal{G}) \land \rfr{probabilitySpace}(\Omega, \mathcal{F}, P_1) \land \rfr{probabilitySpace}(\Omega, \mathcal{F}, P_2)) \implies
\m (\forall_{G \in \mathcal{G}} (P_1(G) = P_2(G)) \implies \forall_{F \in \mathcal{F}} (P_1(F) = P_2(F)))
\n \thm{} \rfr{euclideanSigma}(\sigma_S) = \sigma(\{(-\infty, x] \mid x \in \mathbb{R}\})
