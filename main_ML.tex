...
\n \dfn(\rfr{normalOperator}(A, ())) \iff A^T A = A A^T
\m \eqComment{DEFINE}
...
\n \dfn{event}\ml(F, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\dfn{probabilitySpace}\ml(\ml(\Omega, \mathcal{F}, P\mr), \ml(\mr)\mr)\mr) \land \ml(F \in \mathcal{F}\mr)
...
\n \melazy %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\n \dfn{generatedSigmaAlgebra}(\sigma(\mathcal{M}), (\mathcal{M}, S)) \iff (\forall_{M \in \mathcal{M}} (\rfr{sigmaAlgebra}(M, (S)))) \land 
\m (\rfr{sigmaAlgebra}(\sigma(\mathcal{M}), (S)) = \bigcap (\mathcal{M}))
\m \eqComment{the smallest sigma algebra containing the generating sets}
\n \thm{} (\textbf{cantor set} \cong \mathcal{P}(\mathbb{N}) \land (\mathbb{R}, \rfr{eucledianSigma}, \rfr{lebesgueMeasure})) \implies P(\textbf{cantor set}) = 0 \eqComment{:O}
\n \dfn{conditionalProbability}(P(A | B), (A, B, \Omega, \mathcal{F}, P)) \iff (\rfr{probabilitySpace}(\Omega, \mathcal{F}, P)) \land (A, B \in \mathcal{F}) \land
\m (P(B) > 0) \land (P(A | B) = \frac{P(A \cap B)}{P(B)} \lor P(B) P(A | B) = P(A \cap B)) 
\m \eqComment{calculates $P(A)$ for the subset spanned by $B$}
\m \eqComment{conditioning on 0 probability sets leads to paradoxes}
\n \dfn{independentEvents}((A, B), (\Omega, \mathcal{F}, P)) \iff (A, B \in \mathcal{F}) \land (P(A \cap B) = P(A) P(B))
\m \eqComment{depends on the $P$, not only on $A, B$}
\n \dfn{setPartition}((X)_{\mathbb{N}}, (Y)) \iff (\bigcup_{i \in \mathbb{N}} (X_i) = Y) \land (\forall_{i \in \mathbb{N}} \forall_{j \in \mathbb{N} \setminus \{i\}} (X_i \cap X_j = \emptyset))
\n \thm{} (\rfr{probabilitySpace}(\Omega, \mathcal{F}, P) \land \{A\} \cup (B)_\mathbb{N} \subseteq \mathcal{F}) \implies  
\m (P(A) = \sum_{i \in \mathbb{N}} (P(A | B_i) P(B_i))) \land
\m (\forall_{i \in \mathbb{N}} (P(A | B_i) P(B_i) = P(A) P(B_i | A) = (\sum_{j \in \mathbb{N}} (P(B_i | A) )) P(B_i | A))) \land
\m (P(\bigcap_{i \in \mathbb{N}} (B_i)) = P(B_1) \prod_{i = 2}^{\infty} (P(B_i | \bigcap_{j = 1}^{i - 1} (B_j))))
