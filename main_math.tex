...
\newcommand{\TODO}[1]{\colorbox{lime}{$\displaystyle #1$}}
...
Note: Operators (op)s preserve type; Relations (rel)s return truths; todo rename default funcs
...
\m \TODO{\ml(\rfr{T} \rfr{IF} x \eq x\mr)}
\m \TODO{\ml(\rfr{F} \rfr{IF} x \eq \rfr{T}\mr)}
\m \TODO{\ml(x \rfr{IF} \rfr{T} \eq \rfr{T}\mr)}
\m \TODO{\ml(x \rfr{IF} \rfr{F} \eq \rfr{NOT} x\mr)}  
\m \TODO{\ml(\ml(x \rfr{OR} y\mr) \rfr{IF} z\mr) \eq \ml(x \rfr{IF} z\mr) \rfr{AND} \ml(y \rfr{IF} z\mr)}
\m \TODO{\ml(x \rfr{IF} \ml(y \rfr{AND} z\mr) \eq \ml(x \rfr{IF} y\mr) \rfr{AND} \ml(x \rfr{IF} z\mr)\mr)} 
\m \TODO{\ml(x \rfr{IFF} y \eq x \rfr{IF} y \rfr{AND} \rfr{NOT} x \rfr{IF} \rfr{NOT} y\mr)}
\end{align}
\end{tcolorbox}
...
\TODO{\textbf{undefined terms}:} \defeq, $%
   \dfn{set}, %
   \dfn{tuple}, %
   \dfn{element}, % 
   \dfn{nnumber}, %
