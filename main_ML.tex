...
\n \dfn{finIndEvents}\ml(\ml(A\mr)_{i = 1}^{k}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{probabilitySpace}\ml(\Omega, \mathcal{F}, P\mr)\mr) \land \ml(\forall_{i \in \mathbb{N} \land i \leq k} \ml(A_i \in \mathcal{F}\mr)\mr) \land 
\m \ml(\forall_{I_0 \subseteq \ml(A\mr)_{i = 1}^{k}} \ml(P\ml(\bigcap_{i \in I_0} \ml(A_i\mr)\mr) = \prod_{i \in I_0} \ml(P\ml(A_i\mr)\mr)\mr)\mr)
\m \eqComment{every combination of events must be independent}
...
\m \ml(\forall_{i \in \mathbb{N} \land i \leq k} \ml(A_i \in \mathcal{A}_i\mr)\mr) \land \ml(\rfr{finIndEvents}\ml(\ml(A\mr)_{i = 1}^{k}, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)
...
\n \thm{} \rfr{independentRVs}\ml(\ml(X, Y\mr), \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \forall_{x, y \in \mathbb{R}} \ml(F_{X, Y}\ml(x, y\mr) = F_X\ml(x\mr) F_Y\ml(y\mr)\mr)
\end{align}
\end{tcolorbox}
...
\begin{tcolorbox}[breakable, enhanced]
\begin{align}
\n \textbf{joint discrete part}
\n P_{X | Y}(x | y) = \frac{P_{X, Y}(x, y)}{P_Y(y)} = \frac{P(X = x, Y = y)}{P(Y = y)}
\m \eqComment{picture, works on singletons cos nonzero}
\m \eqComment{PY can be solved by summing of marginal PXs}
\n 0
\end{align}
\end{tcolorbox}
