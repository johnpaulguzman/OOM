...
\n \dfn{seminorm}(\wr\wr \$1 \wr\wr, (+, \cdot, p, M, \sigma, \mu)) \iff (\rfr{vecLp}(\mathcal{L}^p, (+, \cdot, p, M, \sigma, \mu))) \land (\rfr{map}(\wr\wr \$1 \wr\wr, (\mathcal{L}^p, \mathbb{R}))) \land 
...
\n \thm{} \rfr{seminorm}(\wr\wr \$1 \wr\wr, (+, \cdot, p, M, \sigma, \mu)) \implies 
...
\n \dfn{norm}(|| \$1 ||, (+, \cdot, p, M, \sigma, \mu)) \iff (\rfr{seminorm}(|| \$1 ||,(+, \cdot, p, M, \sigma, \mu))) \land (|| f ||= 0 \implies f = \field{0})
\m \eqComment{some of the parameters can be omitted for defining norms on general vector spaces}
\n \dfn{Lp}(L^p, ((+, \cdot, p, M, \sigma, \mu))) \iff (\rfr{seminorm}(\wr\wr \$1 \wr\wr, (+, \cdot, p, M, \sigma, \mu))) \land
\m (L^p = \rfr{quotientSet}(\mathcal{L}^p / \sim, (\mathcal{L}^p, \wr\wr\$1 + (-\$2)\wr\wr = 0))))
