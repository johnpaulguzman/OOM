...
\subsection{Linear algebra segue}
\begin{tcolorbox}[breakable, enhanced]
\begin{align}
   \textbf{matrix: rows=dimensions/vector components, cols=instances/individual vectors}
\n \textbf{matrix mul: rhs dimensions acts on lhs instances}
\n \textbf{gauss-jordan elimination: $E[A | I] = [I | E] = [I | A^{-1}]$}
\n \textbf{elimination: $E[A | b] = [I | E b] = [I | A^{-1} b] = [I | x]$ x in terms of I as basis vectors}
\n \textbf{upper/lower triangular factorization of A: $E A = U \implies A = E^{-1} U = L U$}
\n \textbf{inverse: } (A B)^{-1} (A B) = I = B^{-1} A^{-1} A B 
\n \textbf{transpose: } (A B)_{i, j}^T = (A B)_{j, i} = \sum_{k = 1}^n((a_{j, k}) (b_{k, i})) = \sum_{k = 1}^n((b_{k, i}) (a_{j, k})) = \sum_{k = 1}^n((b_{i, k}^T) (a_{k, j}^T)) = (B^T A^T)
\m \eqComment{INCOMPLETE proof}
\n (A^T)^{-1} = (A^{-1})^T
...
\n Tr(A) = CONTINUE 21
\n A = A^T = A^2 \implies trace(A) = dims(A)
\end{align}
\end{tcolorbox}
