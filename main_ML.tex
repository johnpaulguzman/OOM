...
\n \thm{} a \in [m] \implies [a] = [m] \eqSep [m] = [n] \veebar [m] \cap [n] = \emptyset
...
\n \thm{} \textbf{axiom of choice} \implies \forall_{[m] \in M / \sim} \exists_r \ml(r \in [m]\mr)
...
\m \ml(\exists_{A \in \sigma} \ml(\rfr{nullSet}\ml(A, \ml(M, \sigma, \mu\mr)\mr) \implies \forall_{m \in M \setminus A} \ml(p\ml(m\mr)\mr)\mr)\mr)
...
\n \dfn{nonNegIntegrable}\ml(f, \ml(M, \sigma\mr)\mr) \iff \rfr{measurableMap}\ml(f, \ml(M, \sigma, \overline{\mathbb{R}}, \rfr{exStandardSigma}\mr)\mr) \land
...
\m \ml(\rfr{measurableMap}\ml(f, \ml(M, \sigma, \overline{\mathbb{R}}, \overline{\sigma_{s}}\mr)\mr)\mr) \land (\int_M\ml(f d\mu\mr) = \rfr{sup}(\{\sum_{z \in Z} \ml(z \cdot \mu\ml(\rfr{preimage}\ml(A, \ml(\{z\}, s, M, \mathbb{R}^+_0\mr)\mr)\mr)\mr) \mid
...
\m \thm{Markov inequality} \ml(\forall_{z \in \mathbb{R}^+_0} \ml(\int \ml(f d\mu\mr) \geq z \cdot \mu\ml(\rfr{preimage}\ml(A, \ml(\{z\}, f, M, \overline{\mathbb{R}}\mr)\mr)\mr)\mr)\mr) \land
...
\n 49:05 Section 3: Integrable (without non)
\end{align}
\end{tcolorbox}
