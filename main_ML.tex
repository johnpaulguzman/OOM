...
\n \dfn{inverseOperator}\ml(A^{-1}, \ml(A\mr)\mr) \iff \ml(A^{-1} L = \rfr{identityOperator}\ml(I, \ml(A\mr)\mr)\mr)
...
\n \thm{} (\rfr{matrix}(A, (n, m))) \implies (\rfr{dimensionality}(\rfr{Ker}(A)) = n - \rfr{rank}(r, (A)))
\m \eqComment{number of free variables}
\n \dfn{transposeNorm}(|| x ||, ()) \iff (|| x || = \sqrt{x^T x})
\n \dfn{transposeOrthogonality}((x, y), ()) \iff (|| x ||^2 + || y ||^2 = || x + y ||^2) \iff
\m (x^T x + y^T y = (x + y)^T (x + y) = x^T x + y^T y + x^T y = y^T x) \iff
\m (0 = \frac{x^T x + y^T y - (x^T x + y^T y)}{2} = \frac{x^T y + y^T x}{2} = x^T y) 
\n \dfn{orthogonalProjection}(P_A b, (A, b)) \iff CONTHERE
