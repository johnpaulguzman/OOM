...
TODO should really define union intersection complement etc
...
\n \dfn{homeomorphism}\ml(\phi, \ml(\ml(M, \mathcal{O}_M\mr), \ml(N, \mathcal{O}_N\mr)\mr)\mr) \iff \ml(\rfr{inverseMap}\ml(\phi^{-1}, \ml(\phi, M, N\mr)\mr)\mr)
...
\m \ml({continuous}\ml(f, \ml(\ml(M, \mathcal{O}\mr), \ml([0, 1], {subsetTopology}\ml(\mathcal{O}|_{[0, 1]}, \ml([0, 1], \mathbb{R}, {standardTopology}\mr)\mr)\mr)\mr)\mr)\mr) \land
...
\m \ml(\forall_{p, q \in M} \exists_{\gamma} \ml(continuous\ml(\gamma, \ml(\ml([0, 1], \mathcal{O}_{standard}|_{[0 ,1]}\mr), \ml(M, \mathcal{O}\mr)\mr)\mr) \land \gamma\ml(0\mr) = p \land \gamma\ml(1\mr) = q\mr)\mr)
