...
\n \thm{} \rfr{symmetricOperator}\ml(A^T A, \ml(\mr)\mr) \impliedby \ml(A^T A = \ml(A^T A\mr)^T = A^T A^{TT} = A^T A\mr)
...
\n \dfn{transposeOrthogonality}\ml(\ml(x, y\mr), \ml(\mr)\mr) \iff \ml(|| x ||^2 + || y ||^2 = || x + y ||^2\mr) \iff
\m \ml(x^T x + y^T y = \ml(x + y\mr)^T \ml(x + y\mr) = x^T x + y^T y + x^T y = y^T x\mr) \iff
\m \ml(0 = \frac{x^T x + y^T y - \ml(x^T x + y^T y\mr)}{2} = \frac{x^T y + y^T x}{2} = x^T y\mr) \iff
\m \ml(\sum\ml(x_i y_i\mr) \lor \int\ml(x\ml(u\mr) y\ml(u\mr) du\mr)\mr)
...
\m (\rfr{diagonalOperator}(\Lambda, ()) \{1\}^n = \{\lambda \in \mathbb{R} \mid s \in S^T \land \rfr{eigenvalue}(\lambda, s, A, V)\})
