...
\n \dfn{euclideanSigma}(\sigma_{s}, ()) \iff \ml(\rfr{borelSigmaAlgebra}\ml(\sigma_{s}, \ml(\mathbb{R}^d, \rfr{euclideanTopology}\mr)\mr)\mr)
\n \dfn{lebesgueMeasure}\ml(\lambda, \ml(\mr)\mr) \iff \ml(\rfr{measure}\ml(\lambda, \ml(\mathbb{R}^d, \rfr{euclideanSigma}\mr)\mr)\mr) \land
...
\m \eqComment{in terms of measure, almost nothing is not equivalent to nothing}
\end{align}
\end{tcolorbox}
...
\begin{tcolorbox}[breakable, enhanced]
\begin{align}
   \dfn{simpleTopology}\ml(\mathcal{O}_{simple}, \ml(\mr)\mr) \iff \mathcal{O}_{simple} = \rfr{subsetTopology}\ml(\mathcal{O}|_{\mathbb{R}^+_0}, \ml(\mathbb{R}, \rfr{euclideanTopology}, \mathbb{R}^+_0\mr)\mr)
...
\n \dfn{exEuclideanSigma}\ml(\overline{\sigma_{s}}, \ml(\mr)\mr) \iff \overline{\sigma_{s}} = \{A \subseteq \overline{\mathbb{R}} \mid A \cap R \in \rfr{euclideanSigma}\}
\m \eqComment{ignores $\pm \infty$ to preserve the points in the domain of the measurable map}
\n \dfn{nonNegIntegrable}\ml(f, \ml(M, \sigma\mr)\mr) \iff \ml(\rfr{measurableMap}\ml(f, \ml(M, \sigma, \overline{\mathbb{R}}, \rfr{exEuclideanSigma}\mr)\mr)\mr) \land
...
\m \ml(\rfr{measureSpace}\ml(\ml(\overline{\mathbb{R}}, \rfr{exEuclideanSigma}, \rfr{lebesgueMeasure}\mr), \ml(\mr)\mr)\mr) \land 
...
\m \Bigg(\rfr{metric}\ml(d\ml(\$1, \$2\mr), \ml(V\mr)\mr) \lor \ml(\rfr{map}\ml(d, \ml(V \times V, \mathbb{R}^+_0\mr)\mr)\mr)
\m \ml(\forall_{x, y \in V} \ml(d\ml(x, y\mr) = d\ml(y, x\mr)\mr)\mr) \land
\m \ml(\forall_{x, y \in V} \ml(d\ml(x, y\mr) = 0 \iff x = y\mr)\mr) \land
\m \ml(\forall_{x, y, z \in V} \ml(\ml(d\ml(x, z\mr) \leq d\ml(x, y\mr) + d\ml(y, z\mr)\mr)\mr)\mr)\Bigg)
\m \eqComment{behaves as distances should}
...
\n \dfn{matrix}\ml(L, \ml(n, m\mr)\mr) \iff \ml(\rfr{linearOperator}\ml(L, \ml(\mathbb{R}^m, +_m, \cdot_m, \mathbb{R}^n, +_n, \cdot_n\mr)\mr)\mr)
...
\n \dfn{eigenvalue}\ml(\lambda, \ml(v, L, V, +, \cdot\mr)\mr) \iff \ml(\rfr{eigenvector}\ml(v, \ml(L, V, +, \cdot\mr)\mr)\mr)
\n \dfn{identityOperator}\ml(I, \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, n\mr)\mr)\mr) \land \ml(A I = I A = A\mr)
\n \dfn{inverseOperator}\ml(A^{-1}, \ml(A\mr)\mr) \iff \ml(A^{-1} A = A A^{-1} = I\mr)
\m \eqComment{gauss-jordan elimination: $E[A | I] = [I | E] = [I | A^{-1}]$}
\n CONTHER TODO ABSTRACT ALGEB %https://en.wikipedia.org/wiki/Transpose#Transpose_of_a_linear_map
% apparently adjoint not = transpose
\n \thm{} \ml(A B\mr)^{-1} \ml(A B\mr) = I = B^{-1} A^{-1} A B
\n \dfn{transposeOperator}\ml(A^T, \ml(A\mr)\mr) \iff \ml(\ml(A^T\mr)_{m, n} = \ml(A\mr)_{n, m}\mr) \lor \rfr{adjoint}\ml(A^T, \ml(A\mr)\mr)
\n \dfn{symmetricOperator}\ml(A, \ml(\mr)\mr) \iff \ml(A = \rfr{transposeOperator}\ml(A^T, \ml(A\mr)\mr)\mr) \lor \ml(\rfr{selfAdjoint}\ml(A, \ml(\mr)\mr)\mr)
\n \thm{} \ml(A B\mr)^T = B^T A^T \land \ml(A^T\mr)^{-1} = \ml(A^{-1}\mr)^T
\n \dfn{triangularOperator}\ml(A, \ml(\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, n\mr)\mr)\mr) \land \ml(\forall_{x < n} \forall_{0 < i < x} \ml(A_{i, i} = 0\mr)\mr)
\n \dfn{decomposeLU}\ml(LU\ml(A\mr), \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, n\mr)\mr)\mr) \land \ml(\exists_E \ml(E A = \rfr{triangularOperator}\ml(U, \ml(\mr)\mr)\mr)\mr) \land 
\m \ml(LU\ml(A\mr) = E^{-1} U = A\mr)
\m \eqComment{lower triangle are all 0; useful for solving linear equations}
\n \dfn{Img}\ml(Img\ml(A\mr), \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(Img\ml(A\mr) = \{A v \in \mathbb{R}^n \mid v \in \mathbb{R}^m\}\mr)
\m \eqComment{the column space; not always a subspace since $A$ can map to a set not containing $\field{0}$}
\n \dfn{Ker}\ml(Ker\ml(A\mr), \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(Ker\ml(A\mr) = \{v \in \mathbb{R}^m \mid A v = \field{0} \in \mathbb{R}^n\}\mr)
\m \eqComment{the null or solution space; always a subspace due to linearity $A v + A w = \field{0} = A \ml(v + w\mr)$}
\n \thm{general linear solution} \ml(A x_p = b\mr) \land \ml(x_n \in Ker\ml(A\mr)\mr) \implies \ml(A x_p + A x_n = b + 0 = A \ml(x_p + x_n\mr) = b\mr)
\n \dfn{independentOperator}\ml(A, \ml(\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(\lnot \exists_{v \in \mathbb{R}^m \setminus \field{0}_m} \ml(A v = 0\mr) \iff \rfr{Ker}\ml(A\mr) = \{\field{0}_m\}\mr)
\m \eqComment{also equivalent to invertible operator}
\n \dfn{dimensonality}\ml(N, \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(N = \rfr{inf}\ml(\{|\ml(b\mr)_n| \mid \rfr{basis}\ml(\ml(b\mr)_n, \ml(A\mr)\mr)\}\mr)\mr)
\n \dfn{rank}\ml(r, \ml(A\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(\rfr{dimensonality}\ml(r, \ml(A\mr)\mr)\mr)
\n \thm{} \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \implies \ml(\rfr{dimensionality}\ml(\rfr{Ker}\ml(A\mr)\mr) = n - \rfr{rank}\ml(r, \ml(A\mr)\mr)\mr)
\m \eqComment{number of free variables}
\n \dfn{transposeNorm}\ml(|| x ||, \ml(\mr)\mr) \iff \ml(|| x || = \sqrt{x^T x}\mr)
\n \thm{} P = P^T = P^2
\n \dfn{orthogonalVectors}\ml(\ml(x, y\mr), \ml(\mr)\mr) \iff \ml(|| x ||^2 + || y ||^2 = || x + y ||^2\mr) \iff
\m \ml(x^T x + y^T y = \ml(x + y\mr)^T \ml(x + y\mr) = x^T x + y^T y + x^T y = y^T x\mr) \iff
\m \ml(0 = \frac{x^T x + y^T y - \ml(x^T x + y^T y\mr)}{2} = \frac{x^T y + y^T x}{2} = x^T y\mr) \iff \ml(0 = \sum_{i}\ml(x_i y_i\mr) \lor \int\ml(x\ml(u\mr) y\ml(u\mr) du\mr)\mr)
\m \eqComment{vector and functional orthogonality}
\n \dfn{orthogonalOperator}\ml(Q, \ml(V, +, \cdot, \langle \$1, \$2\rangle\mr)\mr) \iff \ml(\rfr{orthonormalBasis}\ml(Q^T, \ml(V, +, \cdot, \$1^T, \$2\mr)\mr)\mr) \lor \ml(Q^T Q = I\mr) 
\n \thm{} \rfr{orthogonalOperator}\ml(Q, \ml(V, +, \cdot, \langle \$1, \$2\rangle\mr)\mr) \implies \ml(Q^T Q Q^{-1} = I Q^{-1} = Q^T = Q^{-1}\mr)
\n \dfn{orthogonalProjection}\ml(P_A b, \ml(A, b\mr)\mr) \iff \ml(\rfr{matrix}\ml(A, \ml(n, m\mr)\mr)\mr) \land \ml(\rfr{matrix}\ml(b, \ml(m, 1\mr)\mr)\mr) \land
\m \Bigg(\exists_{c \in \mathbb{R}^m} \ml(A^T \ml(b - P_A b\mr) = 0 = A^T \ml(b - A c\mr)\mr) \iff 
\m A^T b = A^T A c \iff c = \ml(A^T A\mr)^{-1} A^T b \iff P_A b = A c = \ml(A \ml(A^T A\mr)^{-1} A^T\mr) b\Bigg)
\m \eqComment{$A, A^T$ may not necessarily be invertible}
\n \thm{} \rfr{independentOperator}(A, ()) \implies \rfr{independentOperator}(A^T A, ())
\n \dfn{eigenvectors}(X, (A, V, +, \cdot, || \$1 ||)) \iff (\rfr{normedVectorSpace}((V, +, \cdot, || \$1 ||), ())) \land 
\m (X = \{v \in V \mid || v || = 1 \land \rfr{eigenvector}(v, (A, V, +, \cdot))\})
\n \dfn{det}(det(A), (A, V, +, \cdot, || \$1 ||)) \iff (\rfr{eigenvectors}(X, (A, V, +, \cdot, || \$1 ||))) \land 
\m (det(A) = \prod_{x \in X} (\rfr{eigenvalue}(\lambda, (x, A, V, +, \cdot))))
\m \eqComment{DEFINE; exterior algebra wedge product area??}
%\m \eqComment{signed area parallelepiped spanned by the operator}
\n \dfn{tr}(tr(A), (A, V, +, \cdot, || \$1 ||)) \iff (\rfr{eigenvectors}(X, (A, V, +, \cdot, || \$1 ||))) \land 
\m (tr(A) = \sum_{x \in X} (\rfr{eigenvalue}(\lambda, (x, A, V, +, \cdot))))
\m \eqComment{DEFINE}
%\m \eqComment{signed perimeter? parallelepiped spanned by the operator} % http://www.onmyphd.com/?p=eigen.decomposition
\n \thm{} \rfr{independentOperator}(A, ()) \iff det(A) \neq 0
\n \thm{} A = A^T = A^2 \implies Tr(A) = \rfr{dimensionality}(N, (A)) \eqComment{counts dimensions}
\n (\rfr{normalOperator}(A, ())) \iff A^T A = A A^T
\m \eqComment{DEFINE}
\n \dfn{diagonalOperator}(A, ()) \iff (\rfr{normalOperator}(A, ())) \land (\rfr{triangularOperator}(A, ())) 
\n \dfn{characteristicEquation}((A - \lambda I) x = 0, (A)) \iff (A x = \lambda x \implies A x - \lambda x = (A - \lambda I) x = 0) \land
\m (x \neq \field{0} \implies \rfr{eigenvalue}(0, (x, A - \lambda I) \implies \prod_{\lambda_i \in \Lambda} = 0 = det(A - \lambda I)))
\m \eqComment{characterizes eigenvalues}
\n \dfn{eigenDecomposition}(S \Lambda S^{-1}, (A, V, +, \cdot, || \$1 ||)) \iff (S \subseteq (\rfr{eigenvectors}(X, (A, V, +, \cdot, || \$1 ||))^T)) \land 
\m (\rfr{diagonalOperator}(\Lambda, ()) \{1\}^n = (\lambda)_n = \{\lambda \in \mathbb{R} \mid s \in S^T \land \rfr{eigenvalue}(\lambda, s, A, V)\})
\m (\rfr{independentOperator}(S, ())) \land (\exists_{S^-1} (A S = S \Lambda \implies A = S \Lambda S^{-1}))
\n \thm{} \rfr{eigenDecomposition}(S \Lambda S^{-1}, (A, V, +, \cdot, || \$1 ||)) \implies A^2 = (A) (A) = S \Lambda S^{-1} S \Lambda S^{-1} = S \Lambda^2 S^{-1}
\n \thm{} \dfn{spectralDecomposition}(Q \Lambda Q^T, (A, V, +, \cdot, || \$1 ||)) \iff (\rfr{symmetricOperator}(A, ())) \implies
\m (\exists_{Q} (\rfr{eigenDecomposition}(Q \Lambda Q^{-1}, (A, V, +, \cdot, \$1^T \$1)) \land \rfr{orthogonalOperator}(Q, (V, +, \cdot, \$1^T \$2)) \land (\lambda)_n \in \mathbb{R}^n))
\m \eqComment{if symmetric and eigenvalues are real, then there exists orthonormal eigenbasis}
\n \dfn{hermitianAdjoint}(A^H, (A)) \iff (A^H = \overline{A}^T) \impliedby (\langle A, A\rangle = \overline{A}^T A \in \mathbb{R})
\m \eqComment{complex analog to adjoint}
\n \dfn{hermitianOperator}(A, ()) \iff A = A^H
\m \eqComment{complex analog to symmetric operator}
\n \dfn{unitaryOperator}(Q^H Q, (Q)) \iff Q^H Q = I
\m \eqComment{complex analog to orthogonal operator}
\n \dfn{positiveDefiniteOperator}(A, (V, +, \cdot, || \$1 ||)) \iff (\forall_{x \in V \setminus \{\field{0}\}} (x^T A x > 0)) \lor
\m (\forall_{x \in \rfr{eigenvectors}(X, (A, V, +, \$1^T \$1))} (\rfr{eigenvalue}(\lambda, (x, A, V, +, \cdot)) \implies \lambda > 0))
\m \eqComment{acts like a positive scalar where any vector only scales and cannot reflect against its perpendicular axis}
\n \thm{} \rfr{positiveDefiniteOperator}(A^T A) \impliedby \forall_{x \in V \setminus \{\field{0}\}} (x^T A^T A x = (A x)^T (A x) = || A x || > 0)
\n \dfn{semiPositiveDefiniteOperator}(A, (V, +, \cdot, || \$1 ||)) \iff (\forall_{x \in V \setminus \{\field{0}\}} (x^T A x \geq 0)) \lor
\m (\forall_{x \in \rfr{eigenvectors}(X, (A, V, +, \$1^T \$1))} (\rfr{eigenvalue}(\lambda, (x, A, V, +, \cdot)) \implies \lambda \geq 0))
\m \eqComment{acts like a nonnegative scalar}
\n \thm{} \rfr{symmetricOperator}(A^T A) \impliedby (A^T A = (A^T A)^T = A^T A^{TT} = A^T A)
\n \dfn{similarOperators}((A, B), ()) \iff (\rfr{matrix}(A, (n, n))) \land (\rfr{matrix}(B, (n, n))) \land (\exists_M (B = M^{-1} A M))
\n \thm{} (\dfn{similarOperators}((A, B), ()) \land A x = \lambda x) \implies (\exists_{M} (M^{-1} A x = \lambda M^{-1} x = M^{-1} A M M^{-1} x = B M^{-1} x)) 
\m \eqComment{similar operators have the same eigenvalues but $M^{-1}$ shifted eigenvectors}
\n \dfn{singularValueDecomposition}(Q \Sigma R^T, (A, V, +, \cdot, \langle \$1, \$2\rangle)) \iff (\rfr{orthogonalOperator}(R, (V, +, \cdot, \$1^T \$2))) \land 
\m (\rfr{orthogonalOperator}(Q, (Img(A), +, \cdot, \$1^T \$2))) \land (\rfr{semiPositiveDefiniteOperator}(\Sigma, (V, +, \cdot, \$1^T \$1))) \land
\m (A R = Q \Sigma) \land (A = Q \Sigma R^{-1} = Q \Sigma R^T) \land (\rfr{symmetricOperator}(A^T A)) \land (\rfr{symmetricOperator}(A A^T)) \land
\m (A^T A = R \Sigma^T Q^T Q \Sigma R^T = R \Sigma^T \Sigma R^T) \land (\rfr{spectralDecomposition}(R (\Sigma^T \Sigma) R^T, (A^T A, V, +, \cdot, \$1^T \$1))) \land
\m (A A^T = Q \Sigma R^T R \Sigma^T Q^T = Q \Sigma \Sigma^T Q^T) \land (\rfr{spectralDecomposition}(Q (\Sigma \Sigma^T) Q^T, (A A^T, V, +, \cdot, \$1^T \$1))) \land
\m (\rfr{diagonalOperator}(\Sigma^T \Sigma) \implies \rfr{normalOperator}(\Sigma^T \Sigma) = \Sigma \Sigma^T = \Sigma_{\sigma^2}) \land (\Sigma = \Sigma_{\sqrt[2]{\sigma^2}} = \Sigma_{|\sigma|})
\m \thm{based on the spectral theorem}
\n \rfr{leftInverseOperator}(A_L^{-1}, (A)) \iff (\rfr{matrix}(A, (n, m))) \land (\rfr{rank}(A) = n < m) \land 
\m (A_L^{-1} A = I = ((A^T A)^{-1} A^T) A)
\n \rfr{rightInverseOperator}(A_R^{-1}, (A)) \iff (\rfr{matrix}(A, (n, m))) \land (\rfr{rank}(A) = m < n) \land 
\m (A A_R^{-1} = I = A (A^T (A A^T)^{-1}))
\end{align}
\end{tcolorbox}
...
\m \ml(\forall_{x \in \mathbb{R}} \ml(P_X\ml(\{x\}\mr) = H_X\ml(x\mr) = P\ml(\{\omega \in \Omega \mid X\ml(\omega\mr) = x\}\mr) = P\ml(X = x\mr)\mr)\mr)
\m \eqComment{decomposition of the probability law for discrete random variables}
...
\n \dfn{geometricRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{discreteRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land
\m \ml(\rfr{countablyInfinite}\ml(E\mr)\mr) \land \ml(p \in \mathbb{R}\mr) \land \ml(\forall_{i \in \mathbb{N}} \ml(P_X\ml(\{e_i\}\mr) = P\ml(X = e_i\mr) = \ml(1-p\mr)^{i - 1} p\mr)\mr)
\n \dfn{binomialRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{discreteRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land
\m \ml(n = |\rfr{finiteSet}\ml(E\mr)|\mr) \land \ml(p \in \mathbb{R}\mr) \land \ml(\forall_{i \in \mathbb{N}} \ml(P_X\ml(\{e_i\}\mr) = P\ml(X = e_i\mr)= {{n}\choose{i}} p^i \ml(1 - p\mr)^{n - i}\mr)\mr)
\n \dfn{poissonRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{discreteRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land
\m \ml(\rfr{countablyInfinite}\ml(E\mr)\mr) \land \ml(\lambda \in \mathbb{R}^+\mr) \land \ml(\forall_{i \in \mathbb{N}} \ml(P_X\ml(\{e_i\}\mr) = P\ml(X = e_i\mr) = \frac{e^{-\lambda} \lambda^i}{i!}\mr)\mr)
\n \dfn{absolutelyContinous}\ml(\ml(f, g\mr), \ml(M, \sigma\mr)\mr) \iff \ml(\rfr{measure}\ml(f, \ml(M, \sigma\mr)\mr)\mr) \land \ml(\rfr{measure}\ml(g, \ml(M, \sigma\mr)\mr)\mr) \land 
\m \ml(\forall_{A \in \sigma} \ml(g\ml(A\mr) = 0 \implies f\ml(A\mr) = 0\mr)\mr)
\n \thm{Radon-Nikodym} \ml(\rfr{measurableSpace}\ml(\ml(M, \sigma\mr), \ml(\mr)\mr)\mr) \land \ml(\rfr{finiteMeasure}\ml(\mu, \ml(M, \sigma\mr)\mr)\mr) \land 
\m \ml(\rfr{finiteMeasure}\ml(\nu, \ml(M, \sigma\mr)\mr)\mr) \land \ml(\rfr{absolutelyContinous}\ml(\ml(\nu, \mu\mr), \ml(M, \sigma\mr)\mr)\mr) \implies 
\m \ml(\exists_{\rfr{map}\ml(f, \ml(M, \overline{\mathbb{R}}^+\mr)\mr)} \forall_{A \in \sigma} \ml(\nu\ml(A\mr) = \int_A \ml(f d\mu\mr)\mr)\mr)
\m \eqComment{connects $P_X = F_X = \int \ml(f_x dx\mr)$}
\n \dfn{continuousRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{randomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land 
\m \ml(\rfr{absolutelyContinous}\ml(\ml(P_X, \rfr{lebesgueMeasure}\mr), \ml(\mathbb{R}, \rfr{euclideanSigma}\mr)\mr)\mr)
\m \eqComment{the probabilities lie on nonzero lebesgue measure sets}
\n \dfn{contUniformRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{continuousRandomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land
\m \ml(a, b \in \mathbb{R}\mr) \land \ml(a < b\mr) \land \ml(P_X = F_X\ml(x\mr) = \begin{cases} 0 & x < a \\ \frac{x}{b-a} & a \leq x \leq b \\ 1 & x > b \end{cases}\mr)
...
\m \ml(\forall_{B \in {\sigma_S}^2} \ml(P_{X, Y}\ml(B\mr) = P\ml(\{\omega \in \Omega \mid \ml(X\ml(\omega\mr), Y\ml(\omega\mr)\mr) \in B\}\mr)\mr)\mr)
...
\m \forall_{x, y \in \mathbb{R}} \ml(F_{X, Y}\ml(x, y\mr) = P\ml(\{\omega \in \Omega \mid X\ml(\omega\mr) \leq x\} \cap \{\omega \in \Omega \mid Y\ml(\omega\mr) \leq y\}\mr) = P\ml(X \leq x, Y \leq y\mr)\mr)
...
\m \ml(\forall_{x_1, x_2, y_1, y_2 \in \mathbb{R}}\ml(\ml(x_1 \leq x_2 \land y_1 \leq y_2\mr) \implies \ml(F_{X, Y}\ml(x_1, y_1\mr) \leq F_{X, Y}\ml(x_2, y_2\mr)\mr)\mr)\mr) \land 
\m \ml(\forall_{x, y \in \mathbb{R}} \ml(\lim_{\substack{\epsilon_x \to 0^+ \\ \epsilon_y \to 0^+}} \ml(F\ml(x + \epsilon_x, y + \epsilon_y\mr) = F\ml(x + y\mr)\mr)\mr)\mr) \land
...
\begin{tcolorbox}[breakable, enhanced]
\begin{align}
   \dfn{preimageSigma}\ml(\sigma\ml(X\mr), \ml(X, \Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{randomVariable}\ml(X, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr) \land 
\m \ml(\sigma\ml(X\mr) = \{A \subseteq \Omega \mid B \in \rfr{euclideanSigma} \land \rfr{preimage}\ml(A, \ml(B, X, \Omega, \mathbb{R}\mr)\mr)\}\mr) \land \ml(\rfr{subSigmaAlgebra}\ml(\sigma\ml(X\mr), \ml(\mathcal{F}, \Omega\mr)\mr)\mr)
\m \eqComment{$X\ml(\omega\mr) \in B$ determines all events $A$ that occurs}
\m OUT OF PLACE TODO overlaps with generated
...
\n \dfn{independentEvents}\ml(\ml(A, B\mr), \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(A, B \in \mathcal{F}\mr) \land \ml(P\ml(A \cap B\mr) = P\ml(A\mr) P\ml(B\mr)\mr)
\m \eqComment{depends on $P, A, B$}
...
\n \dfn{finIndEvents}\ml(\ml(A\mr)_{i = 1}^{k}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{probabilitySpace}\ml(\Omega, \mathcal{F}, P\mr)\mr) \land
\m \ml(\ml(A\mr)_{i = 1}^{k} \subseteq \mathcal{F}\mr) \land \ml(\forall_{I_0 \subseteq \ml(A\mr)_{i = 1}^{k}} \ml(P\ml(\bigcap_{i \in I_0} \ml(A_i\mr)\mr) = \prod_{i \in I_0} \ml(P\ml(A_i\mr)\mr)\mr)\mr)
\m \eqComment{every combination of subsets must be independent}
\n \dfn{arbIndEvents}\ml(\ml(A\mr)_{I}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\forall_{\rfr{finiteSet}\ml(I_F\mr) \subseteq I} \ml(\rfr{finIndEvents}\ml(\ml(A\mr)_{I_F}, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)\mr)
\m \eqComment{every finite subset is independent}
\n \dfn{subSigmaAlgebra}\ml(\mathcal{B}, \ml(\mathcal{F}, \Omega\mr)\mr) \iff \ml(\rfr{sigmaAlgebra}\ml(\mathcal{F}, \ml(\Omega\mr)\mr)\mr) \land \ml(\rfr{sigmaAlgebra}\ml(\mathcal{B}, \ml(\Omega\mr)\mr)\mr) \land \ml(\mathcal{B} \subseteq \mathcal{A}\mr)
\n \dfn{independentSigmaAlgebras}\ml(\ml(\mathcal{A}, \mathcal{B}\mr), \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{probabilitySpace}\ml(\Omega, \mathcal{F}, P\mr)\mr) \land
\m \ml(\rfr{subSigmaAlgebra}\ml(\mathcal{A}, \ml(\mathcal{F}, \Omega\mr)\mr)\mr) \land \ml(\rfr{subSigmaAlgebra}\ml(\mathcal{B}, \ml(\mathcal{F}, \Omega\mr)\mr)\mr) \land 
\m \ml(\forall_{A \in \mathcal{A}} \forall_{B \in \mathcal{B}} \ml(\rfr{independentEvents}\ml(\ml(A, B\mr), \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)\mr)
\n \dfn{finIndSigmaAlgebras}\ml(\ml(\mathcal{A}\mr)_{i = 1}^{k}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\forall_{i \in \mathbb{N} \land i \leq k} \ml(\rfr{subSigmaAlgebra}\ml(\mathcal{A}_i\mr), \ml(\mathcal{F}, \Omega\mr)\mr)\mr) \land
\m \ml(\forall_{i \in \mathbb{N} \land i \leq k} \forall_{A_i \in \mathcal{A}_i} \ml(\rfr{finIndEvents}\ml(\ml(A\mr)_{j = 1}^{k}, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)\mr)
\n \dfn{arbIndSigmaAlgebras}\ml(\ml(\mathcal{A}\mr)_{I}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\forall_{\rfr{finiteSet}\ml(I_F\mr) \subseteq I} \ml(\rfr{finIndSigmaAlgebras}\ml(\ml(\mathcal{A}\mr)_{I_F}, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)\mr)
\m \eqComment{every finite subset is independent}
\n \dfn{independentRV}() CONTHERE %http://theanalysisofdata.com/probability/E_4.html
\n \melazy
...
\m \eqComment{similarly, $\{A_n \text{ i-o}\}$ occur, for all values of $n$, the nth-tail-end event occur}
\n \thm{BCL 1} \ml(\thmlink{}{Cond300} \ml(\sum_{n \in \mathbb{N}} \ml(P\ml(A_n\mr)\mr) < \infty\mr) \implies \ml(P\ml(\{A_n \text{ i-o}\}\mr) = 0\mr)\mr) \impliedby
\m \thmlink{infinitelyOften, MSContU}{1IL300} \ml(P\ml(\bigcap_{n \in \mathbb{N}} \ml(B_n\mr)\mr) = \lim_{n \to \infty} \ml(P\ml(B_n\mr)\mr) = \lim_{n \to \infty} \ml(P\ml(\bigcup_{i = n}^{\infty} \ml(A_i\mr)\mr)\mr)\mr) \land
\m \thmlink{MSSetBount}{2IL300} \ml(\lim_{n \to \infty} \ml(P\ml(\bigcup_{i = n}^{\infty} \ml(A_i\mr)\mr)\mr) \leq \lim_{n \to \infty} \ml(\sum_{i = n}^{\infty} \ml(P\ml(A\mr)_i\mr)\mr)\mr) \land 
\m \thmlink{Cond300}{3IL300} \ml(\lim_{n \to \infty} \ml(\sum_{i = n}^{\infty} \ml(P\ml(A\mr)_i\mr)\mr) = 0\mr) \land \thmlink{1IL300, 2IL300, 3IL300}{Impl300} \ml(0 \leq P\ml(\{A_n \text{ i-o}\}\mr) \leq 0\mr)
\n \thm{} \thmlink{}{logp} \ml(\forall_{x \in [0, 1]} \ml(\log\ml(1 - x\mr) \leq -x\mr)\mr)
\n \thm{} \thmlink{}{sump} \ml(\ml(\thmlink{}{1Cond302} \ml(\forall_{i \in \mathbb{N}} \ml(p_i \in [0, 1]\mr)\mr) \land \thmlink{}{2Cond302} \ml(\sum_{i \in \mathbb{N}} \ml(p_i\mr) = \infty\mr)\mr) \implies \prod_{i \in \mathbb{N}} \ml(1 - p_i\mr) = 0\mr) \impliedby
\m \thmlink{}{1IL302} \ml(\prod_{i \in \mathbb{N}} \ml(1 - p_i\mr) = \exp\ml(\log\ml(\prod_{i \in \mathbb{N}} \ml(1 - p_i\mr)\mr)\mr) = \exp\ml(\log\ml(\lim_{n \to \infty} \ml(\prod_{i = 1}^{n} \ml(1 - p_i\mr)\mr)\mr)\mr)\mr) \land
\m \thmlink{logp}{2IL302} \ml(\exp\ml(\log\ml(\lim_{n \to \infty} \ml(\prod_{i = 1}^{n} \ml(1 - p_i\mr)\mr)\mr)\mr) = \exp\ml(\lim_{n \to \infty} \ml(\sum_{i = 1}^{n} \ml(\log\ml(1 - p_i\mr)\mr)\mr)\mr) \leq \exp\ml(\lim_{n \to \infty} \ml(\sum_{i = 1}^{n} \ml(-p_i\mr)\mr)\mr)\mr) \land
\m \thmlink{2Cond302}{3IL302} \ml(\exp\ml(\lim_{n \to \infty} \ml(\sum_{i = 1}^{n} \ml(-p_i\mr)\mr)\mr) = \exp\ml(-\infty\mr) = 0\mr) \land \thmlink{1Cond302, 1IL302, 2IL302, 3IL302}{Impl302} \ml(0 \leq \prod_{i \in \mathbb{N}} \ml(1 - p_i\mr) \leq 0\mr)
\n \thm{BCL 2} \ml(\ml(\thmlink{}{1Cond303} \ml(\sum_{n \in \mathbb{N}} \ml(P\ml(A_n\mr)\mr) = \infty\mr) \land \thmlink{}{2Cond303} \ml(\rfr{infIndEvents}\ml(\ml(A\mr)_{\mathbb{N}}\mr)\mr)\mr) \implies P\ml(\{A_n \text{ i-o}\}\mr) = 1\mr)
\m \impliedby \thmlink {MSSetBound}{1IL303} \ml(1 - P\ml(\{A_n \text{ i-o}\}\mr) = P\ml(\{A_n \text{ i-o}\}^C\mr) = P\ml(\bigcup_{n \in \mathbb{N}} \ml({B_n}^C\mr)\mr) \leq \sum_{n \in \mathbb{N}} \ml(P\ml({B_n}^C\mr)\mr)\mr) \land
\m \thmlink{DeMorgans, 2Cond303}{2IL303} \ml(\sum_{n \in \mathbb{N}} \ml(P\ml({B_n}^C\mr)\mr) = \sum_{n \in \mathbb{N}} \ml(P\ml(\bigcap_{i = n}^{\infty} \ml({A_i}^C\mr)\mr)\mr) = \sum_{n = 1}^{\infty} \ml(\prod_{i = n}^{\infty} \ml(P\ml({A_i}^C\mr)\mr)\mr) = \sum_{n = 1}^{\infty} \ml(\prod_{i = n}^{\infty} \ml(1 - P\ml(A_i\mr)\mr)\mr)\mr) \land
\m \thmlink{1Cond303, sump}{3IL303}\ml(\sum_{n = 1}^{\infty} \ml(\prod_{i = n}^{\infty} \ml(1 - P\ml(A_i\mr)\mr)\mr) = \sum_{n = 1}^{\infty} \ml(0\mr) = 0\mr) \land \thmlink{1Il303, 2Il303, 3Il303}{Impl303} \ml(0 \leq 1 - P\ml(\{A_n \text{ i-o}\}\mr) \leq 0 \iff P\ml(\{A_n \text{ i-o}\}\mr) = 1\mr)
\n \melazy
\n TODO FIXUP SECTIONING AND FORMATTING
\n \melazy %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
