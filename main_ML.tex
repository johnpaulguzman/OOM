 % CONTINUE: LINE 658 PAGE 20 FIXUP
...
\newcommand{\thmlink}[1]{\text{(THM)} \dfn{#1} \text{ : }}
\makeatother
...
\n \dfn{inverseOperator}\ml(A^{-1}, \ml(A\mr)\mr) \iff \ml(A^{-1} A = \rfr{identityOperator}\ml(I, \ml(A\mr)\mr)\mr)
...
\m (\rfr{diagonalOperator}(\Sigma^T \Sigma) \implies \rfr{normalOperator}(\Sigma^T \Sigma) = \Sigma \Sigma^T = \Sigma_{\sigma^2}) \land (\Sigma = \Sigma_{\sqrt[2]{\sigma^2}})
