...
\n \dfn{getEigenvectors}(X, (A, V, +, \cdot, d(\$1, \$2))) \iff (\rfr{metricVectorSpace}((V, +, \cdot, d(\$1, \$2)), ())) \land 
\m (X = \{v \in V \mid || v || = 1 \land \rfr{eigenvector}(v, (A, V, +, \cdot))\})
\n \dfn{getEigenvalues}(Y, (A, V, +, \cdot, d)) \iff (\rfr{getEigenvectors}(X, (A, V, +, \cdot, d(\$1, \$2)))) \land (Y = \{\lambda \in \mathbb{R} \mid x \in X \land \rfr{eigenvalue}\})
\n CONTHERE FIX MULTIPLICITY IGNORED
\n \dfn{det}(det(A), (A)) \iff (det(A) = ) 
% http://www.onmyphd.com/?p=eigen.decomposition
\n \dfn{invertibleOperator}() \iff 
