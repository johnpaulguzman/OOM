...
\n CONTHERE TODO ABSTRACT ALGEB %https://en.wikipedia.org/wiki/Transpose#Transpose_of_a_linear_map
...
\m \eqComment{maps elementary outcomes to a numeric value and the measurable space to the real line and borel sets}
...
\m \ml(\forall_{x \in \mathbb{R}} \ml(H_X\ml(x\mr) = P\ml(\{\omega \in \Omega \mid X\ml(\omega\mr) = x\}\mr) = P\ml(X = x\mr)\mr)\mr)
\m \eqComment{type of probability law}
\n 00000000000000 SPLIT SECTIONS FIX PMF TO RELATE TO PL
...
\n \dfn{geometricRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{discreteRandomVariable}(X, (\Omega, \mathcal{F}, P))) \land
\m (\rfr{countablyInfinite}(E)) \land (p \in \mathbb{R}) \land (\forall_{i \in \mathbb{N}} (P_X(\{e_i\}) = P(X = e_i) = (1-p)^{i - 1} p))
\n \dfn{binomialRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{discreteRandomVariable}(X, (\Omega, \mathcal{F}, P))) \land
\m (n = |\rfr{finiteSet}(E)|) \land (p \in \mathbb{R}) \land (\forall_{i \in \mathbb{N}} (P_X(\{e_i\}) = P(X = e_i)= {{n}\choose{i}} p^i (1 - p)^{n - i}))
\n \dfn{poissonRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{discreteRandomVariable}(X, (\Omega, \mathcal{F}, P))) \land
\m (\rfr{countablyInfinite}(E)) \land (\lambda \in \mathbb{R}^+) \land (\forall_{i \in \mathbb{N}} (P_X(\{e_i\}) = P(X = e_i) = \frac{e^{-\lambda} \lambda^i}{i!}))
\n \dfn{absolutelyContinous}((f, g), (M, \sigma)) \iff (\rfr{measure}(f, (M, \sigma))) \land (\rfr{measure}(g, (M, \sigma))) \land 
\m (\forall_{A \in \sigma} (g(A) = 0 \implies f(A) = 0))
\n \thm{Radon-Nikodym} (\rfr{measurableSpace}((M, \sigma), ())) \land (\rfr{finiteMeasure}(\mu, (M, \sigma))) \land 
\m (\rfr{finiteMeasure}(\nu, (M, \sigma))) \land (\rfr{absolutelyContinous}((\nu, \mu), (M, \sigma))) \implies 
\m (\exists_{\rfr{map}(f, (M, \overline{\mathbb{R}}^+))} \forall_{A \in \sigma} (\nu(A) = \int_A (f d\mu)))
\m \eqComment{connects $P_X = F_X = \int (f_x dx)$}
\n \dfn{continuousRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{randomVariable}(X, (\Omega, \mathcal{F}, P))) \land 
\m (\rfr{absolutelyContinous}((P_X, \rfr{lebesgueMeasure}), (\mathbb{R}, \rfr{euclideanSigma})))
\m \eqComment{the probabilities lie on nonzero lebesgue measure sets}
\n \dfn{contUniformRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{continuousRandomVariable}(X, (\Omega, \mathcal{F}, P))) \land
\m (a, b \in \mathbb{R}) \land (a < b) \land (P_X = F_X(x) = \begin{cases} 0 & x < a \\ \frac{x}{b-a} & a \leq x \leq b \\ 1 & x > b \end{cases})
\n \dfn{exponentialRandomVariable}(X, (\Omega, \mathcal{F}, P)) \iff (\rfr{continuousRandomVariable}(X, (\Omega, \mathcal{F}, P))) \land
\m (\lambda \in \mathbb{R}^+) \land (P_X = F_X(x) = \begin{cases} 1 - e^{-\lambda x} & x > 0 \\ 0 & x \leq 0 \end {cases})
\n \dfn{memorylessRandomVariable}(X, ()) \iff (\forall_{\omega \in \Omega} (X(\omega) \geq 0)) \land (\forall_{s, t \in \mathbb{R}^+_0} (P(X > s) = P(X > s + t \mid x > t)))
...
\m (\sigma(X) = \{A \subseteq \Omega \mid B \in \rfr{euclideanSigma} \land \rfr{preimage}(A, (B, X, \Omega, \mathbb{R}))\}) \land (\rfr{subSigmaAlgebra}(\sigma(X), (\mathcal{F}, \Omega)))
\m \eqComment{$X(\omega) \in B$ determines all events $A$ that occurs}
\n \dfn{nRandomVariable}((X)_n, (\Omega, \mathcal{F}, P)) \iff (\forall_{i \in \mathbb{N} \land i \leq n} (\rfr{randomVariable}(X_i, (\Omega, \mathcal{F}, P)))) \land 
\m (\rfr{measurableMap}((X)_n, (\Omega, \mathcal{F}, \mathbb{R}^n, \rfr{euclideanSigma}^n)))
\m \eqComment{if the preimage of a measurable set of n dimensional vectors is an event}
\n \thm{} \rfr{PL}(P_{X, Y}, ((X, Y), \Omega, \mathcal{F}, P)) \iff (\forall_{B \in {\sigma_S}^2} (P_{X, Y}(B) = P(\{\omega \in \Omega \mid (X(\omega), Y(\omega)) \in B\})))
\m \eqComment{joint PL}
\n \thm{} \rfr{CDF}(F_{X, Y}, ((X, Y), \Omega, \mathcal{F}, P)) \iff 
\m \forall_{x, y \in \mathbb{R}} (F_{X, Y}(x, y) = P(\{\omega \in \Omega \mid X(\omega) \leq x\} \cap \{\omega \in \Omega \mid Y(\omega) \leq y\}) = P(X \leq x, Y \leq y))
\m \eqComment{joint CDF}
\n \thm{} \rfr{CDF}(F_{X, Y}, ((X, Y), \Omega, \mathcal{F}, P)) \iff (\lim_{\substack{x \to -\infty \\ y \to -\infty}} (F_{X, Y}(x, y)) = 0) \land (\lim_{\substack{x \to \infty \\ y \to \infty}} (F_{X, Y}(x, y)) = 1) \land
\m (\forall_{x_1, x_2, y_1, y_2 \in \mathbb{R}}((x_1 \leq x_2 \land y_1 \leq y_2) \implies (F_{X, Y}(x_1, y_1) \leq F_{X, Y}(x_2, y_2)))) \land 
\m (\forall_{x, y \in \mathbb{R}} (\lim_{\substack{\epsilon_x \to 0^+ \\ \epsilon_y \to 0^+}} (F(x + \epsilon_x, y + \epsilon_y) = F(x + y))))
\m (\forall_{x \in \mathbb{R}} (\lim_{y \to \infty} (F_{X, Y}(x, y)) = F_X(x))) \land (\forall_{y \in \mathbb{R}} (\lim_{x \to \infty} (F_{X, Y}(x, y)) = F_Y(y)))
\m \eqComment{limit evaluation order or trajectory should not matter}
\end{align}
\end{tcolorbox}

\subsection{CONDITIONALS Probability Theory}
\begin{tcolorbox}[breakable, enhanced]
\begin{align}
...
\m \eqComment{calculates $P\ml(A\mr)$ for the subset spanned by $B$}
\m \eqComment{conditioning on 0 probability sets leads to paradoxes}
...
\m \eqComment{depends on the $P$, not only on $A, B$}
...
\n \dfn{finIndEvents}\ml(\ml(A\mr)_{\mathbb{N}_k}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\rfr{probabilitySpace}\ml(\Omega, \mathcal{F}, P\mr)\mr) \land \ml(k \in \mathbb{N}\mr) \land 
\m \ml(A_{\mathbb{N}_k} \subseteq \mathcal{F}\mr) \land \ml(\forall_{I_0 \in \mathcal{P}\ml(\mathbb{N}_k\mr) \setminus \emptyset} \ml(P\ml(\bigcap_{i \in I_0} \ml(A_i\mr)\mr) = \prod_{i \in I_0} \ml(P\ml(A_i\mr)\mr)\mr)\mr)
\m \eqComment{every combination of subsets must be independent}
\n \dfn{infIndEvents}\ml(\ml(A\mr)_{I}, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff 
\m \ml(\forall_{I_F \subseteq I}\ml(\rfr{finiteSet}\ml(I_F\mr) \implies \rfr{finIndEvents}\ml(\ml(A\mr)_{I_F}, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)\mr)
...
\n \dfn{infIndSigmaAlgebras}\ml(\ml(\mathcal{A}\mr)_I, \ml(\Omega, \mathcal{F}, P\mr)\mr) \iff \ml(\forall_{i \in I} \ml(\rfr{subSigmaAlgebra}\ml(\mathcal{A}_i\mr), \ml(\mathcal{F}, \Omega\mr)\mr)\mr) \land
\m \ml(\forall_{i \in I} \ml(F_i \in \mathcal{A}_i\mr)\mr) \land \ml(\rfr{infIndEvents}\ml(\ml(F\mr)_I, \ml(\Omega, \mathcal{F}, P\mr)\mr)\mr)
